...
- [ ] Bare `roonscape` and the source `npm start` workflow launch immediately
...
- [ ] `--config PATH` takes precedence over the standard XDG Display
...
- [ ] Roon Authorization defaults to the XDG state location independently of
...
- [ ] `--help` and `--version` describe the complete public command surface;
      unsupported arguments fail with actionable usage and a nonzero result.
- [ ] The launcher creates private runtime state beneath the owner's XDG runtime
