...
Display Output ID by hand, create and secure a runtime directory, export a
...
Display Output with the arrow keys, offers OLED customization, writes the
...
15. As an owner, I want to choose a Display Output with the arrow keys, so that
    I never have to copy or type an internal output ID.
16. As an owner, I want each choice to show the Display Output and its current
    Display Zone, so that I can distinguish physical outputs accurately.
17. As an owner with identically named outputs, I want the wizard to reveal
    ...
    intended Display Output.
18. As an owner reconfiguring RoonScape, I want the saved Display Output to be
...
20. As an owner, I want Display Output selection to remain read-only with
...
- The output chooser is keyboard-driven. Each item presents the Display Output
  name and current Display Zone. Internal IDs remain storage details and appear
...
- Output discovery remains read-only and never invokes a Roon Control method.
  If no Display Outputs are discoverable, setup offers Refresh and Quit rather
...
- Reconfiguration loads and displays the current Display Output and inactivity
  values. Changing the Display Output preserves inactivity values unless the
...
- First-run tests cover authorization guidance, indefinite waiting with delayed
  troubleshooting text, Retry and Quit, arrow-key Display Output selection,
...
- Reconfiguration tests cover the highlighted current Display Output, prefilled
...
- Manual acceptance on a compatible RoonScape Host verifies that the unpacked
  ...
  Authorization, discovers and selects the intended Display Output, enters the
