...
atomically beside the socket. The renderer uses a neutral palette when artwork
is absent and packaged fallback fonts when the preferred host fonts are not
both available. The live bridge observes Roon's Image and Transport services
and provides extension Status; it does not load Browse, call a Roon Control
method, expose a command endpoint, provide a browser UI, or open a network
listener.
